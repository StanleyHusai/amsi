...
class SLAMAnalysis(QMainWindow): # by timer   For paper NLOS exclusion caused by double-decekr bus
    ...
    def __init__(self, parent=None):
        ...
        rospy.Subscriber('/optimizedOdom', Odometry, self.CallCartoPose)  # This is the pose bias 
    ...
    def CallCartoPose(self,data):
        self.cartoPose = data
        self.cartoPose_.append(0.5+  math.sqrt(self.cartoPose.pose.pose.position.x * self.cartoPose.pose.pose.position.x + self.cartoPose.pose.pose.position.y * self.cartoPose.pose.pose.position.y))
    ...
    def SLAMErrorEvaluation(self):
        ...
        # error to csv
	rows = zip(self.cartoPoseIndex_,self.cartoPose_)
	with open('proposed.csv', "w") as f: # output the integration positioning error
	    writer = csv.writer(f)
	    for row in rows:
		writer.writerow(row)
