...
class pullh2kml_eval():
    def __init__(self):
        rospy.Subscriber('/ekf_loose_Pose', NavSatFix, self.ekf_loose_calback)
        rospy.Subscriber('/ekf_gnss_odom', NavSatFix, self.ekf_gnss_callback)
        rospy.Subscriber('/ekf_span_odom', NavSatFix, self.ekf_span_callback)
    ...
    def ekf_loose_calback(self,data):
    ...
    def ekf_gnss_callback(self,data):
   ...
   def ekf_span_callback(self,data):
        self.uck_bestPos_ = NavSatFix()
        self.uck_bestPos_ = data
        self.ukf_lat_.append(float(self.uck_bestPos_.latitude))
        self.ukf_lon_.append(float(self.uck_bestPos_.longitude))
